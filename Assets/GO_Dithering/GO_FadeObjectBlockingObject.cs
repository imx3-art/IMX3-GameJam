...
public class GO_FadeObjectBlockingObject : MonoBehaviour
{
   [SerializeField]
    ...
    [SerializeField]
    private List<GO_FadingObject> ObjectsBlockingView = new List<GO_FadingObject>();
    private Dictionary<GO_FadingObject, Coroutine> RunningCoroutines = new Dictionary<GO_FadingObject, Coroutine>();
    ...
    private IEnumerator CheckForObjects()
    {
        while (true)
        {
            ...
            if (hits > 0)
            {
                ...
                {
                    GO_FadingObject fadingObject = GetFadingObjectFromHit(Hits[i]);
    ...
    {
        List<GO_FadingObject> objectsToRemove = new List<GO_FadingObject>(ObjectsBlockingView.Count);

        foreach (GO_FadingObject fadingObject in ObjectsBlockingView)
        {
            bool objectIsBeingHit = false;
            for (int i = 0; i < Hits.Length; i++)
            {
                GO_FadingObject hitFadingObject = GetFadingObjectFromHit(Hits[i]);
        ...
        foreach(GO_FadingObject removeObject in objectsToRemove)
        {
            ObjectsBlockingView.Remove(removeObject);
        }
    }

    private IEnumerator FadeObjectOut(GO_FadingObject FadingObject)
    {
    ...
    private IEnumerator FadeObjectIn(GO_FadingObject FadingObject)
    {
        float time = 0;
    ...
    private GO_FadingObject GetFadingObjectFromHit(RaycastHit Hit)
    {
        return Hit.collider != null ? Hit.collider.GetComponent<GO_FadingObject>() : null;
    }
}
